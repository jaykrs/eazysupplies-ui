...
import { horizontalProductSlider, jewelleryCategorySlider } from "@/data/sliderSetting/SliderSetting";
  ...
  return (
    <>
      ...
      {data?.categories?.status && (
        <Container>
          <section className="section-b-space border-section border-top-0 category-rounded section-t-space">
      ...
      {data?.services && (
        <Container className={!data?.products_list?.status ? "section-t-space tools-service service-w-bg" : ""}>
          <section className="pt-0 service section-t-space">

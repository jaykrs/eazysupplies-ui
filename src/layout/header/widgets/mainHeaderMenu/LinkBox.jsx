      ...
      {menu.link_type === "sub" ? (
        ...
        <>
          {menu.link_type == "link" && menu.is_target_blank === 0 ? (
            <Link className="dropdown-item" href={menu?.path.charAt(0) == "/" ? `collections?layout=collection_3_grid&${menu?.path.includes("category") ? "category=" + menu?.id + "&title=" +menu?.title : "brand=" + menu.id + "&title=" +menu?.title }` : `/${menu?.path}`}>
              {menu.title}
